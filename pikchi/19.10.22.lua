...
local function realTime()
  ...
  local connect = internet.request('https://time100.ru')
  local pattern1 = ''
  local pattern2 = ''
  local findPattern = 'class="time"'
  local nowPattern = ''
  local cicl = 0
  while cicl < 10 do
    pattern1 = connect.read(1024)
    nowPattern = pattern1 .. pattern2
    if string.find(nowPattern, findPattern) then
      poss = string.find(nowPattern, findPattern)
      return string.sub(nowPattern, poss + 60, poss + 64)
    else
      pattern2 = pattern1
    end
  end
...
elseif string.find(text[4],"кан игрока") ~= nil and string.find(text[4],"кан игрока") or string.find(text[4],"ня просканируй") or string.find(text[4],"ня что в инвенторе") or string.find(text[4],"ня покажи инвентарь") or string.find(text[4],"ня инвентарь")  then 
...
elseif string.find(text[4],"ня создай иридий") ~= nil and string.find(text[4],"ня создай иридий") or string.find(text[4],"ня скрафти иридий") or string.find(text[4],"ня сделай иридий") or string.find(text[4],"ня закажи иридий") or string.find(text[4],"ня иридий") then 
cb.say("§6Аня§r: §b Поставила на крафт иридий")
rs.setOutput(4, 16)
